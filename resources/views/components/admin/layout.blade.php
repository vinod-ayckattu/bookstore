...
<div class="container mt-5 mb-5">
  <div class="row">
    ...
    <div class="p-3 row bg-info rounded-top-2">
        <div class="col">
        <span>
            <b>Administrator Name:</b> {{ auth()->user()->name }}
        </span>
        </div>
        ...
        <span class="float-end me-2">
           ...
           @endauth
            ...
            <span class="me-1 float-end">
                <a href="{{ route('admin.stockrequests') }}" class="ms-2 text-decoration-none">
                    <button class="btn btn-sm bg-success text-white rounded-5">Stock Requests</button>
                </a>
            </span>
        <span class="float-end me-1">
            <form action="{{ route('admin.bookrequests') }}">
            <button class="btn btn-sm btn-success rounded-5">New Book Requests</button>
            </form>
        </span>
        <span class="float-end me-1">
            <form action="{{ route('admin.orders') }}">
            <button class="btn btn-sm btn-success rounded-5">Orders</button>
            </form>
        </span>
        <span class="float-end me-1">
            <form action="{{ route('admin.index') }}">
            <button class="btn btn-sm btn-success rounded-5">Home</button>
            </form>
        </span>
        </div>
    </div>
